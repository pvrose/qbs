...
void QBS_history::create_form() {
	...
	chart_->textsize(FL_NORMAL_SIZE);
...
void QBS_history::copy_data_to_chart() {
	...
	chart_->set_axis_ranges(1, zc_range<double>(0, 100), zc_range<double>(0, 150), zc_range<double>(0, 100));
	...
	for (int b = 0; b <= total_boxes; b++) {
		...
		sent_data_.push_back({ b, sum_sent * 100.0 / sum_received });
		recycled_data_.push_back({ b, sum_recycled * 100.0 / sum_received });
	};
